...
public class Lobby : MonoBehaviour
{
    ...
    private async void RefreshLobbies()
    {
        ...
        options.Filters = new List<QueryFilter>{
            ...
            {
                var joingLobbyInstance = await Lobbies.Instance.JoinLobbyByIdAsync(lobby.Id);
                string joinCode = joingLobbyInstance.Data["JoinCode"].Value;
                int currentPlayerCount = joingLobbyInstance.Players.Count;
                int maxPlayerCount = lobby.MaxPlayers;
                VisualElement lobbyContainer = CreateLobbyContainer("Unknown Lobby Name", joinCode, currentPlayerCount, maxPlayerCount);
                _listLobbiesScrollView.Add(lobbyContainer);
            }
    }
